...
namespace KaikeiSystemBody.models.db.tables
{
    ...
    /// 予算（科目）
    /// </summary>
    class YosanKamoku : AbstractTable
    {
        public const string TABLE_NAME = "yosan_kamoku";

        public YosanKamoku(AbstractDatabase db, string tableName = TABLE_NAME)
            : base(db, tableName) {
        ...
        override public void LoadCSV(string filename) {
            using (var reader = new System.IO.StreamReader(filename, System.Text.Encoding.GetEncoding("shift_jis"))) {
                ...
                while ((line = reader.ReadLine()) != null) {
                    ...
                    row.Set("kamoku_id", CSVParser.ReadInt(token[4]));
